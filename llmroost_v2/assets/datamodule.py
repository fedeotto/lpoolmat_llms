...
def create_dataset(df, elem_attrs,desc):
    ...
    for i, (f,t,struct_fam,chem_fam) in enumerate(zip(df['formula'],
                                                      df['target'],
                                                      df['StructuralFamily'],
                                                      df['ChemicalFamily'])):
        comp_graphs.append(create_material_data(f,t,elem_attrs=elem_attrs,struct_fam=struct_fam,chem_fam=chem_fam))
    return comp_graphs